...
import { DexInstructions, INSTRUCTION_LAYOUT } from './instructions';
...
export class Market {
  ...
  async getTokenAccountsByOwnerForMint(
    ...
    const response: any = await connection.getTokenAccountsByOwner(ownerAddress, {
      mint: mintAddress,
    });
  
    // Convert the response to a mutable array
    const accounts: Array<{ pubkey: PublicKey; account: AccountInfo<Buffer> }> = [...response.value];
  ...
  async loadFills(connection: Connection, limit = 100) {
    ...
    return events
      .filter(
        (event: any) => event.eventFlags.fill && event.nativeQuantityPaid.gtn(0),
      )
...
function getPriceFromKey(key: any) {
  return key.ushrn(64);
}
...
async function getFilteredProgramAccounts(
  ...
  return resp.result.map(
    ({ pubkey, account: { data, executable, owner, lamports } }: { pubkey: string, account: { data: string[], executable: boolean, owner: string, lamports: number } }) => ({
